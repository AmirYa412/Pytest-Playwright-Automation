...
class InventoryPage(BasePage):
    ...
    def get_product_title(self, product_name: str):
        ...
        return self.get_product_card(product_name).locator(self.item_name)
    ...
    def get_product_price(self, product_name: str):
        ...
        return self.get_product_card(product_name).locator(self.item_price)
    ...
    def are_items_sorted_as_expected(self, sort_by: str) -> bool:
        ...
        """
        if sort_by in ("az", "za"):
            names = [element.text_content() for element in self.item_name.all()]
            return names == sorted(names, reverse=(sort_by == "za"))

        if sort_by in ("lohi", "hilo"):
            prices = [float(element.text_content().replace("$", "")) for element in self.item_price.all()]
